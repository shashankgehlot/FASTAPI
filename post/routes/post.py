...
def create_post(post: PostCreate, current_user: UserSchema = Depends(get_current_user)):
    ...
    try:
        ...
        if post.thumbnail_base64:
            new_post.set_thumbnail(post.thumbnail_base64)
        if post.post_image_base64:
            new_post.set_post_image(post.post_image_base64)
    ...
    return PostResponse(
        ...
        thumbnail_base64=new_post.thumbnail_base64,
        post_image_base64=new_post.post_image_base64
    )
...
def get_post(post_id: str):
    ...
    return PostResponse(
        ...
        thumbnail_base64=post.thumbnail_base64,
        post_image_base64=post.post_image_base64
    )
...
def get_tagged_posts(tags: Optional[List[str]] = Query(None)):
    if tags:
        tags_objects = Tags.objects(title__in=tags)
        tags_ids = [tag.id for tag in tags_objects]
        posts = Post.objects(tags__in=tags_ids)
    else:
        posts = Post.objects()
    response = []
    for post in posts:
...
def update_post(post_update: PostUpdate, post: Post = Depends(can_update_or_delete_post)):
    ...
    if post_update.tags:
        for tag_data in post_update.tags:
            ...
            post.add_tags(tag_data.title)
    if post.thumbnail_base64:
            post.set_thumbnail(post.thumbnail_base64)
    if post.post_image_base64:
            post.set_post_image(post.post_image_base64)
